...
class AgentSlashCommandTests(unittest.TestCase):
    ...
    def test_search_commands_render_and_update_local_search_runtime(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            off_result = agent.run('/search off')
            disabled_result = agent.run('/search alpha query')
            on_result = agent.run('/search on')
            context_result = agent.run('/search context high')
            toggle_result = agent.run('/search toggle')
        ...
        self.assertIn('web_search_enabled=False', off_result.final_output)
        self.assertIn('Search failed: Web search is disabled', disabled_result.final_output)
        self.assertIn('web_search_enabled=True', on_result.final_output)
        self.assertIn('web_search_context_size=high', context_result.final_output)
        self.assertIn('web_search_enabled=False', toggle_result.final_output)
