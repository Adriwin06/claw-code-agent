...
| 🆕 | **MCP Transport** | Real stdio and Streamable HTTP MCP transport for `initialize`, resource listing/reading, and tool listing/calling |
...
| 🛰️ **MCP Runtime** | Local MCP manifests plus real stdio and Streamable HTTP MCP transport for resources and tools |
...
- [x] Local MCP runtime: manifest resources, portable stdio transport, Streamable HTTP transport, MCP resources, and MCP tool calls
...
- [ ] Full MCP service parity beyond the current working stdio + Streamable HTTP transport and local manifest/resource/tool support
